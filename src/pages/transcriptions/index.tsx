...
import { getTranscriptions, deleteTranscriptionById } from 'apiSdk/transcriptions';
import { TranscriptionInterface } from 'interfaces/transcription';

type ColumnType = ColumnDef<TranscriptionInterface, unknown>;
...
export function TranscriptionListPage(props: TranscriptionListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<TranscriptionInterface>>(
    () => `/transcriptions?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: TranscriptionInterface) => {
    if (hasAccess('transcription', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/transcriptions/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/transcriptions/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('transcription', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/transcriptions/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('transcription', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="transcription" />
        </Flex>

        {hasAccess('transcription', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/transcriptions/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
