...
import { getTranscriptionById } from 'apiSdk/transcriptions';
import { TranscriptionInterface } from 'interfaces/transcription';

function TranscriptionViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<TranscriptionInterface>(
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('transcription', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/transcriptions/edit/${id}`} passHref legacyBehavior>
                    <Button
