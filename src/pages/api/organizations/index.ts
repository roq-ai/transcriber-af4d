...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    const body = { ...req.body };
    if (body?.additional_service?.length > 0) {
      const create_additional_service = body.additional_service;
      body.additional_service = {
        create: create_additional_service,
      };
    } else {
      delete body.additional_service;
    }
    if (body?.payment_plan?.length > 0) {
      const create_payment_plan = body.payment_plan;
      body.payment_plan = {
        create: create_payment_plan,
      };
    } else {
      delete body.payment_plan;
    }
