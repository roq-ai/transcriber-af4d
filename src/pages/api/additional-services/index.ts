...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    ...
    case 'POST':
      return createAdditionalService();
    default:
  ...
  async function getAdditionalServices() {
    ...
    const response = await prisma.additional_service
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'additional_service'),
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function createAdditionalService() {
    ...
    const data = await prisma.additional_service.create({
      data: body,
    });
