...
import { getTranscriptions } from 'apiSdk/transcriptions';
import { TranscriptionInterface } from 'interfaces/transcription';
import { getVideos } from 'apiSdk/videos';
import { VideoInterface } from 'interfaces/video';
import { getAdditionalServices } from 'apiSdk/additional-services';
import { AdditionalServiceInterface } from 'interfaces/additional-service';

import { AdditionalServiceListPage as ShowCaseList } from 'pages/additional-services';

function DashboardPage() {
  const { session } = useSession();

  const transcriptionStat = useSWR<PaginatedInterface<TranscriptionInterface>>(
    () => `/transcriptions`,
    () => getTranscriptions({ limit: 1, offset: 0 }),
  );

  const videoStat = useSWR<PaginatedInterface<VideoInterface>>(
    () => `/videos`,
    () => getVideos({ limit: 1, offset: 0 }),
  );

  const additionalServiceStat = useSWR<PaginatedInterface<AdditionalServiceInterface>>(
    () => `/additional-services`,
    () => getAdditionalServices({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Transcriptions',
      result: transcriptionStat?.data?.totalCount,
      isLoading: transcriptionStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Videos',
      result: videoStat?.data?.totalCount,
      isLoading: videoStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Additional Services',
      result: additionalServiceStat?.data?.totalCount,
      isLoading: additionalServiceStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Additional Services
                </Text>
                <NextLink href={`additional-services`} passHref legacyBehavior>
                  <Button
